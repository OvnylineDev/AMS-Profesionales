 ...
 * SOURCE: MS SQL
 */
define('MSSQL_HOST','OVNYLINE04\SQLEXPRESS');
define('MSSQL_USER','Ovnyline');
define('MSSQL_PASSWORD','AMS2016ovnyline');
define('MSSQL_DATABASE','GestionExpedientes');
/*
 * DESTINATION: MySQL
 */
...
define('MYSQL_DATABASE','ams_middle2');
/*
 * STOP EDITING!
 */

define('DB_TABLES', array(  'TbUsuarios',
                            'TbTiposFichero',
                            'TbOperarios',
                            'TbExpedientes',
...
// Connect MS SQL
$mssql_connect = @sqlsrv_connect(MSSQL_HOST, array("Database"=>MSSQL_DATABASE, "UID"=>MSSQL_USER, "PWD"=>MSSQL_PASSWORD)) or die("Couldn't connect to SQL Server on '".MSSQL_HOST."'' user '".MSSQL_USER."'\n");
...
$mssql_tables = array();
// Get MS SQL tables
$sql = "SELECT * FROM ".MSSQL_DATABASE.".sys.Tables;";
echo $sql;
$res = sqlsrv_query($mysql_connect, $sql);
echo "\n=> Getting tables..\n";
...
if (!empty($mssql_tables))
{
	...
	foreach ($mssql_tables as $table)
	{
		...
		if ($res) 
		{
			...
			$qres = sqlsrv_query($mysql_connect, $sql);
