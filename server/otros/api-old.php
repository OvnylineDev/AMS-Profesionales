...
{
	$operation = $_GET['op'];
     $usernameop = $_GET['usuario'];
	$passop = $_GET['clave'];
}
if(isset($_REQUEST) && $_REQUEST!=null)
{
	$operation = $_REQUEST['op'];
     $usernameop = $_REQUEST['usuario'];
	$passop = $_REQUEST['clave'];
}
...
switch($operation)
{
	...
	/*case "send_mail": 	include("./sendmail.php");
     ...
     case 'login': {
		...
		$query = "SELECT IdOperario, Usuario, PassWord FROM ".$bbdd.".dbo.TbOperarios WHERE inactivo='0' AND Usuario = '".$usernameop."'";

		$getUser = $conn->query($query);

		if($getUser === false)
		{
			echo json_encode(array( 'status'=>'KO', 'error'=>'1', 'message'=>utf8_encode(formatError(msql_error())) ));
			return false;
		}

		if($conn->num_rows($getUser)>1)
		{
			$rowCount = $conn->num_rows($getUser);
			if($rowCount>1)
			{
				echo json_encode(array( 'estado'=>'KO', 'error'=>'2', 'message'=>'Más de un usuario con el mismo identificador' ));
				return false;
			}
			...
			{
				echo json_encode(array('estado'=>'OK', 'usuario'=>$row['Usuario'], 'token'=>''));
			}
			else
			{
				echo json_encode(array( 'estado'=>'KO', 'error'=>'3', 'message'=>'No coinciden los datos de acceso' ));
				return false;
			}
		}
		else
		{
			echo json_encode(array( 'estado'=>'KO', 'error'=>'4', 'message'=>'No existe el usuario' ));
			return false;
		}
	...
	// case 'inicio': {
     //      $autorizacion=check_authorization();
	//
	// 	if($autorizacion[0]==0)
	// 	{
	// 		$conn=connectDB();
