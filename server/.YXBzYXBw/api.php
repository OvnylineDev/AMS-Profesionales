...
switch($operation){
	...
	case 'login':
		...
		if($rowCount>=1){
			...
			if($row['Usuario']==$usernameop && strtolower($row['PassWord'])==$passop){
				...
				saveOp($token, $mobtoken, $row['IdOperario'], $row['Usuario'], $row['Gremio'], $row['Subcontrata']);
	...
	case 'pendiente':
	     ...
	     $query = makeQueryExpPend($idoperario);
	...
	case 'completado':
	     ...
	     $query = makeQueryExpComp($idoperario);
	...
	case 'contactos':
		...
		if ($fechaCita == "") {
			$fechaCita = dia_sem($date->format(DAY_WEEK)) . ", " . $date->format(DATE_FOR_CLI);
		}
	...
	case 'seguimiento':
		...
		$query = "SELECT 	Id,
						...
						fecharegistro,
						Descripcion
				FROM 	TbExpedientesFicheros
				WHERE 	IdSerie LIKE '".$idSerie."'
					AND	NumeroOrden LIKE '".$numeroOrden."'
					AND	idoperario LIKE '".$idOperario."'
				ORDER BY fecharegistro DESC";

		$getFichExp = $conn->query($query);
		...
		// 		$fecha = dia_sem($date->format(DAY_WEEK)) . ", " . $date->format(DATE_FOR_CLI);
		//
		// 		$output[] = array(
		// 			'id'				=>trim($row['id']),
		// 			'idSerie'			=>trim($row['idserie']),
		...
		if($conn->num_rows($getFichExp)>0){

			while (($row = mssql_fetch_array($getFichExp))){

				// $observaciones = rtftophp($row['Observaciones']);

				$date = new DateTime(trim($row['fecharegistro']));
				$fecha = dia_sem($date->format(DAY_WEEK)) . ", " . $date->format(DATE_FOR_CLI);
				...
				$ficheros[] = array(
					...
					'Fecha'			=>$fecha,
					'date'			=>$date,
					'Observaciones'	=>trim($row['Descripcion']),
					'fichero'			=>true
					// 'Observaciones'	=>$observaciones
					// 'Observaciones'	=>"<span style='font-size: 11px;'>se creo para prueba test<\/span><p>"
				 );
		...
		if (count($expedientes) != 0) {
			if (count($ficheros) != 0) {
				...
				$output = orderMultiDimensionalArray(array_merge($expedientes, $ficheros), 'date', true);
			}else{
				$output = array_merge($expedientes);
			}
	...
	case 'seguiurg':
	     ...
	     $query = "SELECT 	IdRegistro,
						...
						FechaRegistro,
						DescripcionFichero
				FROM 	TbFicheros_Incidencias_Festivos
				WHERE 	IdOperario LIKE '".$idOperario."'
				ORDER BY FechaRegistro DESC";
				// ORDER BY Fecha ASC";

	     $getFichInci = $conn->query($query);

		if($getFichInci === false)
	     {
			//Error 1 - Base de datos
	          echo json_encode(array( 'status'=>'KO', 'error'=>'1', 'message'=>utf8_encode($conn->see_errors()) ));
	          return false;
		...
		if($conn->num_rows($getFichInci)>0){

			while (($row = mssql_fetch_array($getFichInci))){

				// $observaciones = rtftophp($row['Observaciones']);
				$idIncidencia = trim($row['ReferenciaExpediente']);

				if (!in_array($idIncidencia, $incidencias)) {
				...
				$ficheros[] = array(
					'id'				=>trim($row['IdRegistro']),
					'idIncidencia'		=>$idIncidencia,
					'IdOperario'		=>trim($row['IdOperario']),
					'Fecha'			=>$fecha,
					'date'			=>$date,
					'Observaciones'	=>trim($row['DescripcionFichero'])
					// 'Observaciones'	=>$observaciones
					// 'Observaciones'	=>"<span style='font-size: 11px;'>se creo para prueba test<\/span><p>"
				);
			}
		...
		if (count($ficheros) != 0) {
			echo json_encode(array('status'=>'OK', 'items'=>$ficheros, 'incid'=>$incidencias, 'token'=>$token));
		     return true;
		}else{
			//Error 4 - La búsqueda no da resultados
	          echo json_encode(array('status'=>'KO', 'error'=>'4', 'message'=>'No hay seguimientos'));
	...
	*/
	...
	case 'idIncidencia':
		...
		$idIncidencia = uniqid("INCI_", false);
...
function makeQueryExpPend($idoperario){
	...
	$query = "SELECT 	expedest.id,
			...
			WHERE 	exped.IdSerie = expedest.IdSerie
				...
				AND	expedest.FechaCita IS NOT NULL
				AND 	expedest.IdOperario LIKE '".$idoperario."'
			ORDER BY  expedest.FechaCita " . $orden;

	return $query;
}

function makeQueryExpComp($idoperario){

	$fincita = "Fincita=1";
	$orden = "DESC";
	$intervalo = 90; //intervalo en días
	...
	$query = "SELECT 	expedest.id,
			...
			WHERE 	exped.IdSerie = expedest.IdSerie
				...
				AND	expedest.FechaCita IS NOT NULL
				AND 	expedest.IdOperario LIKE '".$idoperario."'
				AND FechaCita BETWEEN  GETDATE () - ".$intervalo." AND GETDATE ()
			ORDER BY  expedest.FechaCita " . $orden;
...
function saveOp($token, $mobtoken, $IdOperario, $nombre, $gremio, $subcont){
	// Añadir al json la información
	$arr_seg = array(
          $id => array(
			"token" => $token,
