	...
	case 'login':
		...
		if($rowCount>=1){
			...
			if($row['Usuario']==$usernameop && strtolower($row['PassWord'])==$passop){
				...
				saveOp($token, $row['IdOperario'], $row['Usuario'], $row['Gremio'], $row['Subcontrata']);
			...
			$admins = getFullJSON("admin");

			foreach ($admins as $admin) {
				$adminid = $admin["id"];
				$adminname = strtolower($admin["name"]);
				$adminpass = trim (decrypt($admin["pass"]));
				if ($adminname == $usernameop && $adminpass == $passop) {
					$token = bin2hex(openssl_random_pseudo_bytes(10));

					saveOp($token, $adminid, $adminname, "admin", false);
		...
		if (count($fileurls)>0) {
				...
				if($local_file){
					...
					if(!$connftp->ov_ftp_upload($remote_folder, $remote_file, $local_file, true)){
						...
						if (!$funciona) {
							...
							$jsonIdFich = uniqid("fich-", true);
							saveFich($jsonIdFich, $idSerie, $numeroOrden, $idOperario, $descr, $rutaRemota, $local_file, $tipofichero, $jsonIdSeg);
			...
			if ($message=="") {
				...
				if ($funcionaseg) {
					...
					saveSeg($jsonIdSeg, $idSerie, $numeroOrden, $idOperario, $message);
...
//TODO - Carpeta ficheros *****
	case 'crearparte':
		...
		if ($pdfurl!="") {
			...
			if ($message=="") {
				...
				if ($funcionaseg) {
					// Añadir al json la información
					saveSeg($jsonIdSeg, $idSerie, $numeroOrden, $idOperario, $messeg);
	...
	case 'segllamadasubc':
		...
		if ($subc && $cambiosubc) {
			...
			$fechaCitaOld = str_replace('+', ' ', $fechaCitaOld);
			$fechaCitaNew = str_replace('+', ' ', $fechaCitaNew);

			$query = makeQuerySeg($idSerie, $numeroOrden, $idOperario, $observaciones);
			$funcionaseg = $conndb->query($query);

			$query = makeQuerySeg($idSerie, $numeroOrden, $idOperario, "El operario subcontrata ha introducido un cambio en la fecha de cita, de " . $fechaCitaOld . " a " . $fechaCitaNew);
			$funcionaseg = $conndb->query($query);
			...
			if ($funcionaseg) {
				...
				// saveExpCita($idExpEst, $jsonIdSeg, $idSerie, $numeroOrden, $idOperario, $fechaCitaOld, $fechaCitaNew);
				saveExpCita($idExpEst, $jsonIdSeg, $idSerie, $numeroOrden, $idOperario, $fechaCitaOld, $fechaCitaNew, false);
	...
	case 'adminsegoper':
	     ...
	     $seguimientos = getSearchJSON('seguimiento', array('IdSerie' => $idSerie, 'NumeroOrden' => $numeroOrden));
		$operarios = getFieldsJSON('operario', array('IdOperario', 'nombre', 'gremio'));

		$idOperExp = array();

		foreach ($seguimientos as $seguimiento) {
			array_push($idOperExp, $seguimiento['IdOperario']);
		}

		$idOperExp = array_unique($idOperExp, SORT_REGULAR);

		$operExp = array();

		foreach($operarios as $operario){
			foreach ($idOperExp as $idOp) {
				if ($operario['IdOperario'] == $idOp) {
					array_push($operExp, $operario);
				}
			}
		}
		...
		if(count($operExp) != 0){
	          echo json_encode(array('status'=>'OK', 'items'=>$operExp, 'token'=>$token));
	          return true;
	     }else{
			//Error 5 - No hay expedientes que mostrar
	          echo json_encode(array('status'=>'KO', 'error'=>'5', 'message'=>"No hay seguimientos almacenados"));
	...
	case 'adminsegopersegs':
	     ...
	     $seguimientos = getSearchJSON('seguimiento', array('IdSerie' => $idSerie, 'NumeroOrden' => $numeroOrden, 'IdOperario' => $idOperario));
	     // $seguimientos = array('IdSerie' => $idSerie, 'NumeroOrden' => $numeroOrden, 'idOperario' => $idOperario);
	...
	case 'adminshowadj':
	     ...
	     $adjuntosJSON = getSearchJSON('fichero', array('idSeg' => $idseg, 'tipoFichero' => $adjuntos));

		$adjuntosURL = array();

		foreach ($adjuntosJSON as $adjunto) {
			array_push($adjuntosURL, array('tipofichero' => $adjunto["tipoFichero"], 'url' => $adjunto["rutaLocal"], 'descripcion' => $adjunto["Descripcion"]));
		}
		...
		if(count($adjuntosURL) != 0){
	          echo json_encode(array('status'=>'OK', 'items'=>$adjuntosURL, 'token'=>$token));
	          return true;
	     }else{
			//Error 5 - No hay expedientes que mostrar
	          echo json_encode(array('status'=>'KO', 'error'=>'5', 'message'=>"No hay seguimientos almacenados"));
	...
	case 'admgest':
	     ...
	     {
	          $idSeg = $_POST['idSeg'];
	          $gestionado = ($_POST['gestionado'] == "true");
	          $admin = $_POST['admin'];
	          $token = $_POST['token'];
	     }

		$seguimientos = readJSON('seguimiento');
		$tempArray = array();
		...
		if ($seguimientos != null) {
		     foreach ($seguimientos as $iter => &$subarray) {
		          foreach ($subarray as $id => &$seguimiento) {
		              	if ($id == $idSeg) {
						$temp .= $id . " ";
		              		$seguimiento["Gestionado"] = $gestionado;
		              		$seguimiento["Admin"] = $admin;
		              	}
		          }
		     }
		}

		$funciona = writeFullJSON($seguimientos, 'seguimiento');

	     if($funciona && $seguimientos == null)
	     {
			//Error 1 - Base de datos
	          echo json_encode(array( 'status'=>'KO', 'error'=>'7', 'message'=>'El acceso al JSON no funcionó' ));
	          return false;
		...
		if(count($seguimientos) != 0){
	          echo json_encode(array('status'=>'OK', 'message'=>'Cambio realizado ' . $temp, 'token'=>$token));
	          return true;
	     }else{
			//Error 5 - No hay expedientes que mostrar
	          echo json_encode(array('status'=>'KO', 'error'=>'5', 'message'=>"No hay seguimientos almacenados"));
	...
	*/
	...
	// 	$conndb = new OV_SQLConnect();
     //
	// 	$observaciones = "Fin de cita solicitado por el operario";
     //
	// 	$query = makeQuerySeg($idSerie, $numeroOrden, $idOperario, $observaciones);
	// 	$funcionaseg = $conndb->query($query);
     //
	...
	// 		// Añadir al json la información
	// 		saveSeg($jsonIdSeg, $idSerie, $numeroOrden, $idOperario, $observaciones);
	// 		saveExpCita($idExpEst, $jsonIdSeg, $idSerie, $numeroOrden, $idOperario, "", "", true);
     //
	// 		echo json_encode(array(	'status'=>'OK','message'=>"Seguimiento registrado "));
	// 	}else{
	// 		//Error 1 - Base de datos
...
function saveOp($token, $IdOperario, $nombre, $gremio, $subcont){
	// Añadir al json la información
	$arr_seg = array(
          $id => array(
			"token" => $token,
...
function saveFich($id, $idSerie, $numeroOrden, $idOperario, $Descripcion, $rutaRemota, $rutaLocal, $tipoFichero, $idSeg){
	// Añadir al json la información
	$arr_fich = array(
          $id => array(
			"id" => $id,
...
function writeJSON($array, $tipo){
	if ($tipo != UNKNOWN) {
		...
		return file_put_contents($json_file, $json_string, LOCK_EX);
	}
